          ...
          <form action="http://127.0.0.1/Sites/Intranet-chat/addPosts" method="post" class="sectionPublichUser sectionPubliNotifis" accept-charset="utf-8">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
              <select name="id_tipo_evento">
                ...
                @foreach ($GetNotificaciones as $keyGetNotificaciones) 
                  <option value="{{ $keyGetNotificaciones->id }}" style="background: {{ $keyGetNotificaciones->color_evento }}; color:white" >{{ $keyGetNotificaciones->descripcion_evento }}</option>
                @endforeach
              </select>
            </div>
